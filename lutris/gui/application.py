...
class Application(Gtk.Application):
    ...
    def __init__(self):
        ...
        GObject.add_emission_hook(Game, "game-install-update",
                                  self.on_game_install_update)
        GObject.add_emission_hook(Game, "game-install-dlc",
                                  self.on_game_install_dlc)
    ...
    def on_app_window_destroyed(self, app_window, window_key):
        ...
        try:
            ...
            logger.info("Available windows: %s",
                        ", ".join(self.app_windows.keys()))
        return True
    ...
    def on_game_install(self, game):
        ...
        if installers:
            ...
            ErrorDialog(
                _("There is no installer available for %s.") % game.name,
                parent=self.window,
            )
        return True
    ...
    def print_game_json(self, command_line, game_list):
        games = [{
            ...
            "hidden":
            bool(game["hidden"]),
            "playtime": (str(timedelta(
                hours=game["playtime"])) if game["playtime"] else None),
    ...
    def uninstall_runner(self, runner):
        if "wine" in runner:
            print(
                "Are sure you want to delete Wine and all of the installed runners?[Y/N]"
            )
            ans = input()
    ...
    def install_wine_cli(self, version):
        ...
        runner_path = os.path.join(
            WINE_DIR, f"{version}{'' if '-x86_64' in version else '-x86_64'}")
