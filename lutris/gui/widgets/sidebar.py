...
class SidebarRow(Gtk.ListBoxRow):
    ...
    def create_button_box(self):
        ...
        for action in self.get_actions():
            btn = Gtk.Button(tooltip_text=action[1],
...
class ServiceSidebarRow(SidebarRow):
    ...
    def get_actions(self):
        ...
        return [("view-refresh-symbolic", _("Reload"), self.on_refresh_clicked,
                 "refresh")]
...
class OnlineServiceSidebarRow(ServiceSidebarRow):
    ...
    def get_buttons(self):
        return {
            "run": (("media-playback-start-symbolic", _("Run"),
                     self.on_service_run, "run")),
            "refresh": (
...
class RunnerSidebarRow(SidebarRow):
    ...
    def get_actions(self):
        ...
        if self.runner.runnable_alone:
            entries.append(("media-playback-start-symbolic", _("Run"),
                            self.runner.run, "run"))
        entries.append((
...
class LutrisSidebar(Gtk.ListBox):
    ...
    def get_sidebar_icon(self, icon_name):
        name = icon_name if has_stock_icon(
            icon_name) else "package-x-generic-symbolic"
