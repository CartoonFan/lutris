...
RUNNER_DIR = sio.read_setting("runner_dir") or os.path.join(
    DATA_DIR, "runners")
RUNTIME_DIR = sio.read_setting("runtime_dir") or os.path.join(
    DATA_DIR, "runtime")
CACHE_DIR = sio.read_setting("cache_dir") or os.path.join(
    GLib.get_user_cache_dir(), "lutris")
...
if "nosetests" in sys.argv[0] or "nose2" in sys.argv[
        0] or "pytest" in sys.argv[0]:
    PGA_DB = "/tmp/pga.db"
else:
...
STEAM_API_KEY = sio.read_setting(
    "steam_api_key") or "34C9698CEB394AB4401D65927C6B3752"
DISCORD_CLIENT_ID = sio.read_setting(
    "discord_client_id") or "618290412402114570"

SHOW_MEDIA = (os.environ.get("LUTRIS_HIDE_MEDIA") != "1"
              and sio.read_setting("hide_media") != "True")
