...
class NineManager:
    ...
    def is_available():
        ...
        return NineManager.nine_is_supported(
        ) and NineManager.nine_is_installed()
        ...
        for nine_file in NineManager.nine_files:
            for lib in system.LINUX_SYSTEM.iter_lib_folders():
                ...
                if (os.path.exists(nine_file_path)
                        and CabInstaller.get_arch_from_dll(
                            nine_file_path) == "win32"):
                    shutil.copy(nine_file_path, self.get_system_path("x32"))

                if self.wine_arch == "win64" and (
                        os.path.exists(nine_file_path) and CabInstaller.
                        get_arch_from_dll(nine_file_path) == "win64"):
    ...
    def move_dll(self, backup):
        ...
        wine_dll_path = os.path.join(self.get_system_path("x32"),
                                     NineManager.nine_dll + ".dll")
        if os.path.exists(wine_dll_path +
                          src_suff) and not os.path.islink(wine_dll_path +
                                                           src_suff):
        ...
        if self.wine_arch == "win64":
            wine_dll_path = os.path.join(self.get_system_path("x64"),
                                         NineManager.nine_dll + ".dll")
            if os.path.exists(wine_dll_path +
                              src_suff) and not os.path.islink(wine_dll_path +
                                                               src_suff):
